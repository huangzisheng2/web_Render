...
parser.add_argument('-i', action="store_true", default=False, help=u'查看起运时间和年份信息')
...
# 计算大运方向和起运时间
year_gan_seq = Gan.index(gans.year)
...
# 计算起运岁数
def calculate_qiyun_age(lunar, is_male, is_yang_year):
    """
    计算起运岁数
    ...
    """
    # 获取出生时间到上一个和下一个节气的天数
    prev_jieqi = lunar.getPrevJieQi(True)
    ...
    if (is_male and is_yang_year) or (not is_male and not is_yang_year):
        # 顺行大运：出生后到下一个节气的时间差
        # 使用lunar_python的Julian日计算
        birth_julian = birth_solar.getJulianDay()
        next_julian = next_jieqi_solar.getJulianDay()
        days = int(next_julian - birth_julian)
    else:
        # 逆行大运：出生前到上一个节气的时间差
        birth_julian = birth_solar.getJulianDay()
        prev_julian = prev_jieqi_solar.getJulianDay()
        days = int(birth_julian - prev_julian)
    ...
    if birth_hour >= 23:
        ...
        shichen = 9  # 酉时
    elif birth_hour >= 21 and birth_hour < 23:
        shichen = 10  # 戌时
    else:
        shichen = 11  # 亥时
...
# 计算起运时间
qiyun_years, qiyun_months = calculate_qiyun_age(lunar, not options.n, is_yang_year)

# 计算小运
def generate_xiaoyuns(year_gan_zhi, count=20):
    """
    生成小运列表
...
# 生成小运
xiaoyuns = generate_xiaoyuns(gans.year + zhis.year, count=qiyun_years + 1)
...
if options.b:
    ...
    if options.i:
        ...
        print("【起运时间分析】")
        print("="*120)
        print(f"大运方向: {'顺行' if direction == 1 else '逆行'}")
        print(f"年柱阴阳: {'阳年' if is_yang_year else '阴年'}")
        print(f"性别: {'男命' if not options.n else '女命'}")
        print(f"起运岁数: {qiyun_years}岁{qiyun_months}个月")
        print(f"起运说明: {'顺行大运，计算出生后到下一个节气的时间差' if ((not options.n and is_yang_year) or (options.n and not is_yang_year)) else '逆行大运，计算出生前到上一个节气的时间差'}")
        print("\n上一个节气:", lunar.getPrevJieQi(True), "时间:", lunar.getPrevJieQi(True).getSolar().toYmdHms())
        print("下一个节气:", lunar.getNextJieQi(True), "时间:", lunar.getNextJieQi(True).getSolar().toYmdHms())
        print("="*120)

        # 显示小运信息（0岁到起运岁数）
        if qiyun_years > 0:
            print("\n" + "="*120)
            print("【小运信息】（童限阶段：0岁到{}岁）".format(qiyun_years))
            print("="*120)
            print("序号\t年龄\t小运\t\t\t说明")
            print("-"*120)
            for xy in xiaoyuns:
                if xy['age'] <= qiyun_years:
                    note = "童限" if xy['age'] < qiyun_years else "起运"
                    print(f"{xy['age']}\t{xy['age']}岁\t{xy['gan_zhi']}\t\t{note}")
            print("="*120)
        ...
        print("【大运年份信息】")
        print("="*120)
        print("序号\t大运\t\t起运年龄\t\t结束年龄\t\t对应年份范围")
        print("-"*120)

        # 估算出生年份（根据lunar.getYear()）
        birth_year = lunar.getYear()

        for i, dayun in enumerate(yun.getDaYun()[1:]):
            ...
            gan = dayun.getGanZhi()[0]
            zhi = dayun.getGanZhi()[1]

            print(f"{i+1}\t{gan}{zhi}\t\t{start_age}岁\t\t{end_age-1}岁\t\t{start_year}年-{end_year}年")
        print("="*120)
...
# 第六论级：大运流年对原局的影响分析
def analyze_sixth_level(gans, zhis, dayuns, yun, options, me):
    """
    第六论级：将大运、流年带入原局，分析其对原局组合的引发、加强、破坏或转化
    
    重复第二论级、第三论级、第四论级的分析，将大运流年带入原局
    ...
    # 获取大运天干地支
    dayun_gan = Gan[Gan.index(gans.month) + direction] if direction == 1 else Gan[Gan.index(gans.month) - 1]
    dayun_zhi = Zhi[Zhi.index(zhis.month) + direction] if direction == 1 else Zhi[Zhi.index(zhis.month) - 1]

    # 获取流年
    current_year = solar.getYear()
    liunian_yun = yun.getDaYun()
    current_dayun = None
    for dayun_item in liunian_yun[1:]:
        dayun_start_age = dayun_item.getStartAge()
        dayun_end_age = dayun_start_age + 10
        if dayun_start_age <= (current_year - lunar.getYear()) < dayun_end_age:
            current_dayun = dayun_item
            break
    if not current_dayun:
        current_dayun = liunian_yun[1]

    liunians = current_dayun.getLiuNian()
    current_liunian = None
    for ln in liunians:
        if ln.getYear() == current_year:
            current_liunian = ln
            break
    if not current_liunian and len(liunians) > 0:
        current_liunian = liunians[0]

    if current_liunian:
        liunian_gan = current_liunian.getGanZhi()[0]
        liunian_zhi = current_liunian.getGanZhi()[1]
    else:
        liunian_gan = None
        liunian_zhi = None

    # 构建包含大运、流年的扩展局
    extended_gans = list(gans) + [dayun_gan] + ([liunian_gan] if liunian_gan else [])
    extended_zhis = list(zhis) + [dayun_zhi] + ([liunian_zhi] if liunian_zhi else [])

    print(f"\n【原局八字】")
    print(f"年柱：{gans[0]}{zhis[0]}  月柱：{gans[1]}{zhis[1]}  日柱：{gans[2]}{zhis[2]}  时柱：{gans[3]}{zhis[3]}")
    print(f"\n【大运流年】")
    print(f"大运：{dayun_gan}{dayun_zhi}  流年：{liunian_gan}{liunian_zhi} ({current_year}年)")
    print("="*120)

    # ============== 重复第二论级：地支关系分析（将大运流年带入原局） ==============
    print("\n" + "="*120)
    print("【第六论级-第二论级：地支关系分析（含大运流年）】")
    print("="*120)

    print(f"\n原局地支：{zhis}")
    print(f"大运地支：{dayun_zhi}")
    print(f"流年地支：{liunian_zhi}")
    print("-"*120)

    # 分析大运地支与原局地支的关系
    print("\n【大运地支与原局地支的关系】")
    if dayun_zhi in zhi_atts:
        for i, orig_zhi in enumerate(zhis):
            for type_name in ['合', '冲', '刑', '害']:
                if type_name in zhi_atts[dayun_zhi]:
                    related_zhis = zhi_atts[dayun_zhi][type_name]
                    if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                        impact_type = '引发' if type_name == '合' else '破坏' if type_name == '冲' else '牵制'
                        print(f"  大运{dayun_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}{type_name} → {impact_type}原局")

    # 分析流年地支与原局地支的关系
    if liunian_zhi and liunian_zhi in zhi_atts:
        print("\n【流年地支与原局地支的关系】")
        for i, orig_zhi in enumerate(zhis):
            for type_name in ['合', '冲', '刑', '害']:
                if type_name in zhi_atts[liunian_zhi]:
                    related_zhis = zhi_atts[liunian_zhi][type_name]
                    if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                        impact_type = '引发' if type_name == '合' else '破坏' if type_name == '冲' else '牵制'
                        print(f"  流年{liunian_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}{type_name} → {impact_type}原局")

    # 检查大运流年是否形成三会、三合、六合
    print("\n【三会、三合、六合分析】")
    for i, orig_zhi in enumerate(zhis):
        # 三合
        if dayun_zhi in zhi_3hes and orig_zhi in zhi_3hes[dayun_zhi]:
            third_zhi = set(zhi_3hes[dayun_zhi]) - {dayun_zhi, orig_zhi}
            if third_zhi:
                print(f"  大运{dayun_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}三合为{list(third_zhi)[0]} → 加强原局")

        if liunian_zhi and liunian_zhi in zhi_3hes and orig_zhi in zhi_3hes[liunian_zhi]:
            third_zhi = set(zhi_3hes[liunian_zhi]) - {liunian_zhi, orig_zhi}
            if third_zhi:
                print(f"  流年{liunian_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}三合为{list(third_zhi)[0]} → 加强原局")

        # 三会
        for sanhui_key, sanhui_value in zhi_huis.items():
            if dayun_zhi in sanhui_value and orig_zhi in sanhui_value:
                third_zhi = set(sanhui_value) - {dayun_zhi, orig_zhi}
                if third_zhi:
                    print(f"  大运{dayun_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}三会为{list(third_zhi)[0]} → 引发原局")

            if liunian_zhi and liunian_zhi in sanhui_value and orig_zhi in sanhui_value:
                third_zhi = set(sanhui_value) - {liunian_zhi, orig_zhi}
                if third_zhi:
                    print(f"  流年{liunian_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}三会为{list(third_zhi)[0]} → 引发原局")
        ...
        for zhi_pair, (he_name, wuxing) in liuhe_pairs.items():
            if zhi_pair[0] == dayun_zhi and zhi_pair[1] == orig_zhi:
                print(f"  大运{dayun_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}六合为{wuxing}局 → 加强原局")
            if liunian_zhi and zhi_pair[0] == liunian_zhi and zhi_pair[1] == orig_zhi:
                print(f"  流年{liunian_zhi}与原局{['年','月','日','时'][i]}柱{orig_zhi}六合为{wuxing}局 → 加强原局")

    # ============== 重复第三论级：天干关系分析（将大运流年带入原局） ==============
    print("\n" + "="*120)
    print("【第六论级-第三论级：天干关系分析（含大运流年）】")
    print("="*120)

    print(f"\n原局天干：{gans}")
    print(f"大运天干：{dayun_gan}")
    print(f"流年天干：{liunian_gan}")
    print("-"*120)

    # 分析大运天干与原局天干的关系
    print("\n【大运天干与原局天干的关系】")
    for i, orig_gan in enumerate(gans):
        if (dayun_gan, orig_gan) in gan_hes or (orig_gan, dayun_gan) in gan_hes:
            print(f"  大运{dayun_gan}与原局{['年','月','日','时'][i]}柱{orig_gan}相合 → 转化原局")

        # 检查相生关系
        if gan5[dayun_gan] in wuhangs[gan5[orig_gan]]:
            print(f"  大运{dayun_gan}生助原局{['年','月','日','时'][i]}柱{orig_gan} → 加强原局")
        elif gan5[orig_gan] in wuhangs[gan5[dayun_gan]]:
            print(f"  大运{dayun_gan}被原局{['年','月','日','时'][i]}柱{orig_gan}克制 → 削弱原局")

    # 分析流年天干与原局天干的关系
    if liunian_gan:
        print("\n【流年天干与原局天干的关系】")
        for i, orig_gan in enumerate(gans):
            if (liunian_gan, orig_gan) in gan_hes or (orig_gan, liunian_gan) in gan_hes:
                print(f"  流年{liunian_gan}与原局{['年','月','日','时'][i]}柱{orig_gan}相合 → 转化原局")

            if gan5[liunian_gan] in wuhangs[gan5[orig_gan]]:
                print(f"  流年{liunian_gan}生助原局{['年','月','日','时'][i]}柱{orig_gan} → 加强原局")
            elif gan5[orig_gan] in wuhangs[gan5[liunian_gan]]:
                print(f"  流年{liunian_gan}被原局{['年','月','日','时'][i]}柱{orig_gan}克制 → 削弱原局")

    # ============== 重复第四论级：特殊格局分析（将大运流年带入原局） ==============
    print("\n" + "="*120)
    print("【第六论级-第四论级：特殊格局分析（含大运流年）】")
    print("="*120)
    ...
    if (dayun_gan, dayun_zhi) in gaitou_combinations:
        print(f"  大运{dayun_gan}{dayun_zhi}为盖头 → 可能形成特殊格局")
    if (dayun_gan, dayun_zhi) in jiejiao_combinations:
        print(f"  大运{dayun_gan}{dayun_zhi}为截脚 → 可能破坏原有格局")

    if liunian_gan and liunian_zhi:
        if (liunian_gan, liunian_zhi) in gaitou_combinations:
            print(f"  流年{liunian_gan}{liunian_zhi}为盖头 → 可能形成特殊格局")
        if (liunian_gan, liunian_zhi) in jiejiao_combinations:
            print(f"  流年{liunian_gan}{liunian_zhi}为截脚 → 可能破坏原有格局")
    ...
    # 分析大运流年对日主的影响
    print(f"\n日主为{me}")
    dayun_shen = ten_deities[me].get(dayun_gan, '')
    liunian_shen = ten_deities[me].get(liunian_gan, '') if liunian_gan else ''

    print(f"\n大运天干{dayun_gan}对日主：{dayun_shen if dayun_shen else '无'}")
    if liunian_gan:
        print(f"流年天干{liunian_gan}对日主：{liunian_shen if liunian_shen else '无'}")

    # 综合判断
    print("\n【综合判断】")
    # 判断大运流年对原局是引发、加强、破坏还是转化
    impact_count = 0
    strengthen_count = 0
    weaken_count = 0
    transform_count = 0
    ...
    for i, orig_zhi in enumerate(zhis):
        if dayun_zhi in zhi_atts:
            if '合' in zhi_atts[dayun_zhi]:
                related_zhis = zhi_atts[dayun_zhi]['合']
                if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                    strengthen_count += 1
            if '冲' in zhi_atts[dayun_zhi]:
                related_zhis = zhi_atts[dayun_zhi]['冲']
                if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                    weaken_count += 1

        if liunian_zhi and liunian_zhi in zhi_atts:
            if '合' in zhi_atts[liunian_zhi]:
                related_zhis = zhi_atts[liunian_zhi]['合']
                if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                    strengthen_count += 1
            if '冲' in zhi_atts[liunian_zhi]:
                related_zhis = zhi_atts[liunian_zhi]['冲']
                if orig_zhi in (related_zhis if isinstance(related_zhis, (list, tuple)) else [related_zhis]):
                    weaken_count += 1

    impact_count = strengthen_count + weaken_count + transform_count

    print(f"\n  引发次数：{transform_count}")
    print(f"  加强次数：{strengthen_count}")
    print(f"  破坏次数：{weaken_count}")
    print(f"  影响总计：{impact_count}")

    if strengthen_count > weaken_count:
        print(f"\n  综合评估：大运流年主要{['引发','加强'][0]}原局组合，整体运势较好")
    elif weaken_count > strengthen_count:
        print(f"\n  综合评估：大运流年主要{['破坏','削弱'][0]}原局组合，需谨慎行事")
    else:
        print(f"\n  综合评估：大运流年对原局影响平衡，吉凶参半")

    # 喜忌分析
    print(f"\n喜用神分析：")
    me_wuxing = gan5[me]
    if me_wuxing == '金':
        print(f"  - 日主为金，喜土生助，喜火锻炼，忌木耗泄，忌水寒湿")
    elif me_wuxing == '木':
        print(f"  - 日主为木，喜水滋养，喜火生长，忌金克伐，忌土埋没")
    elif me_wuxing == '水':
        print(f"  - 日主为水，喜金生助，喜木泄秀，忌土克制，忌火蒸发")
    elif me_wuxing == '火':
        print(f"  - 日主为火，喜木生助，喜土晦养，忌水克灭，忌金耗泄")
    elif me_wuxing == '土':
        print(f"  - 日主为土，喜火生助，喜金泄秀，忌木克伐，忌水淹没")
...
if not options.b:
    try:
        analyze_sixth_level(gans, zhis, dayuns, yun, options, me)
